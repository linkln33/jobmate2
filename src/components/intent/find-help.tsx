...
const FindHelp: React.FC<FindHelpProps> = ({ onSelectCategory, onBack, onNext }) => {
  const theme = useTheme();
  const [selectedCategoryId, setSelectedCategoryId] = useState<string | null>(null);
  
  // Get categories for the 'find-help' intent
  const categories = getCategoriesForIntent('find-help');
  ...
  const handleNext = () => {
    if (selectedCategoryId) {
      onNext('find-help', selectedCategoryId);
    }
  };
  ...
  return (
    <Box sx={{ py: 4 }}>
      ...
      <Typography variant="body1" align="center" color="text.secondary" sx={{ mb: 4 }}>
        Select a category that best matches your needs
      </Typography>
