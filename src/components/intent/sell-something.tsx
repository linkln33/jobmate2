...
const SellSomething: React.FC<SellSomethingProps> = ({ onSelectCategory, onBack, onNext }) => {
  const theme = useTheme();
  const [selectedCategoryId, setSelectedCategoryId] = useState<string | null>(null);
  
  // Get categories for the 'sell-something' intent
  const categories = getCategoriesForIntent('sell-something');
  ...
  const handleNext = () => {
    if (selectedCategoryId) {
      onNext('sell-something', selectedCategoryId);
    }
  };
  ...
  return (
    <Box sx={{ py: 4 }}>
      ...
      <Typography variant="body1" align="center" color="text.secondary" sx={{ mb: 2 }}>
        Select a category for your items
      </Typography>
          ...
          {popularCategories.map((catId) => {
            ...
            return category ? (
              <Chip 
                ...
                color={selectedCategoryId === catId ? "primary" : "default"}
                variant={selectedCategoryId === catId ? "filled" : "outlined"}
                sx={{ m: 0.5 }}
              />
            ) : null;
          })}
